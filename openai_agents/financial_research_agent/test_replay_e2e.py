"""End-to-end test for OTEL tracing with replay.

This test verifies that the financial research workflow works correctly
with max_cached_workflows=0, which forces replay on every workflow task.
"""
...
@workflow.defn
class ResearchWorkflowWithSpans:
...
class TestReplayEndToEnd:
    """End-to-end tests for replay behavior."""

    @pytest.mark.asyncio
    async def test_multi_activity_workflow_with_replay(self, tracing: InMemorySpanExporter):
        """
        Test a workflow with multiple activities and replay.
        ...
        async with await WorkflowEnvironment.start_local() as env:
            ...
            async with Worker(
                ...
                workflows=[ResearchWorkflowWithSpans],
                ...
                with tracer.start_as_current_span("client_request") as root:
                    root.set_attribute("query", "Test financial query")
                    result = await client.execute_workflow(
                        ResearchWorkflowWithSpans.run,
